...
const UserForm = ({ mode = 'add', user = null, onSuccess, onCancel }) => {
    ...
    useEffect(() => {
        if (mode === 'edit' && user) {
            ...
            setFormData({
                ...
                medicalHistory: user.medicalHistory && user.medicalHistory.length > 0
            ...
            if (user.medicalHistory && user.medicalHistory.length > 0) {
                ...
                user.medicalHistory.forEach((history, index) => {
                    if (history.prescriptionUrls?.length || history.otherAttachmentUrls?.length) {
    ...
    const handleMedicalHistoryNestedChange = (index, nestedField, field, value) => {
        setFormData(prev => ({
            ...prev,
            medicalHistory: prev.medicalHistory.map((item, i) => {
                if (i === index) {
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            ...
            const balagruhaSelector = document.querySelector('.form-balagruha-selector');
        ...
        try {
            ...
            if (formData.role !== "admin") {
                if (formData.balagruhaIds && formData.balagruhaIds.length > 0) {
                    const balagruhaIdsList = formData.balagruhaIds.map(bg => bg._id).join(',');
            ...
            if (formData.role === 'student') {
                ...
                if (formData.assignedMachines && formData.assignedMachines.length > 0) {
                    const machineIdsList = formData.assignedMachines.map(machine => machine._id).join(',');
                ...
                formData.medicalHistory.forEach((history, index) => {
                    ...
                    formDataToSend.append(`medicalHistory[${index}].description`, history.description);
                    formDataToSend.append(`medicalHistory[${index}].date`, history.date);
                    formDataToSend.append(`medicalHistory[${index}].caseId`, history.caseId);
                    formDataToSend.append(`medicalHistory[${index}].doctorsName`, history.doctorsName);
                    formDataToSend.append(`medicalHistory[${index}].hospitalName`, history.hospitalName);

                    // Add nested currentStatus fields
                    formDataToSend.append(`medicalHistory[${index}].currentStatus.status`, history.currentStatus.status);
                    formDataToSend.append(`medicalHistory[${index}].currentStatus.notes`, history.currentStatus.notes);
                    formDataToSend.append(`medicalHistory[${index}].currentStatus.date`, history.currentStatus.date);

                    // Add files if available
                    if (files.medicalHistoryFiles[index]?.prescriptions) {
                        files.medicalHistoryFiles[index].prescriptions.forEach(file => {
                            formDataToSend.append(`medicalHistory[${index}].prescriptions`, file);
                        });
                    }

                    if (files.medicalHistoryFiles[index]?.otherAttachments) {
                        files.medicalHistoryFiles[index].otherAttachments.forEach(file => {
                            formDataToSend.append(`medicalHistory[${index}].otherAttachments`, file);
                        });
                    }
                ...
                <div className="form-section">
                            ...
                            <div className="form-group">
                                ...
                                <div className="password-input-group">
                                    <input
                                        ...
                                        placeholder={mode === 'add' ? "Enter New Password" : "Retype to reset passoword"}
                                    />
                                    <button
                                        type="button"
                    ...
                    <div className="form-group">
                        ...
                        <div className="status-toggle">
                            ...
                            <label className={formData.status === 'inactive' ? 'inactive' : ''}>
                                <input
                                    type="radio"
                                    name="status"
                                    value="inactive"
...

                    {
                        ...
                        <div className="form-group">
                            ...
                            <div className="form-balagruha-selector">
                                <div
                                    className={`form-dropdown-header ${errors.balagruhaIds ? 'form-error' : ''}`}
                                    onClick={() => setDropdownOpen(prev => !prev)}
                                >
                                    <span>
                                        {formData.balagruhaIds.length
                                            ? `${formData.balagruhaIds.map(bg => bg.name).join(', ')}`
                                            : 'Select Balagruha'}
                                    </span>
                                    <span className="form-dropdown-arrow">{dropdownOpen ? '▲' : '▼'}</span>
                                </div>
                                {dropdownOpen && (
                                            ...
                                            <label key={option._id} className="form-checkbox-option">
                                                <input
                                                    ...
                                                    checked={formData.role === 'student'
                                                        ? formData.balagruhaIds.some(bg => bg._id === option._id)
                                                        : formData.balagruhaIds.some(bg => bg._id === option._id)
                                                    }
                                                    onChange={(e) => {
                                                        if (formData.role === 'student') {
                                                            ...
                                                            const isSelected = formData.balagruhaIds.some(bg => bg._id === option._id);
                                                            const selectedBalagruhas = isSelected
                                                                ? formData.balagruhaIds.filter(bg => bg._id !== option._id)
                            ...
                            {errors.balagruhaIds && <span className="form-error-message">{errors.balagruhaIds}</span>}
                        </div>
                    }
                    ...
                    <div className="form-section">
                        ...
                        <div className="form-group">
                            ...
                            <div className="form-machine-selector">
                                <div
                                    className={`form-dropdown-header ${errors.assignedMachines ? 'form-error' : ''}`}
                                    onClick={() => setMachineDropdownOpen(!machineDropdownOpen)}
                                >
                                    <span>
                                        {formData.assignedMachines.length
                                            ? `${formData.assignedMachines.map(machine => machine.machineId).join(', ')}`
                                            : 'Select Machines'}
                                    </span>
                                    <span className="form-dropdown-arrow">{machineDropdownOpen ? '▲' : '▼'}</span>
                                </div>
                                {machineDropdownOpen && (
                                    <div className="form-dropdown-options">
                                        {formData.balagruhaIds.length > 0 ? (
                                            ...
                                            formData.balagruhaIds.map(balagruha => {
                                                const selectedBalagruha = balagruhaOptions.find(bg => bg._id === balagruha._id);


                                                const availableMachines = selectedBalagruha?.assignedMachines?.filter(machine => {
                                                                ...
                                                                onChange={() => {
                                                                    setFormData(prev => ({
                                                                        ...
                                                                        assignedMachines: isChecked
                                                                            ? prev.assignedMachines.filter(m => m._id !== machine._id)
                            ...
                            {errors.assignedMachines && <span className="form-error-message">{errors.assignedMachines}</span>}
                        </div>
                        ...
                        <div className="form-group">
                            ...
                            {errors.parentalStatus && <span className="error-message">{errors.parentalStatus}</span>}
                        </div>

                        {
                            ((formData.parentalStatus && formData.parentalStatus) === "has one" || (formData.parentalStatus && formData.parentalStatus) === "has guardian") && (
                                <>
                                    <div className="form-group">
                                        <label htmlFor="guardianContact">{formData.parentalStatus === "has one" ? 'Parent Name' : 'Guardian Name'} *</label>
                                        <input
                                            type="text"
                                            id="guardianName1"
                                            name="guardianName1"
                                            ...
                                            placeholder={formData.parentalStatus === "has one" ? 'Parent Name' : 'Guardian Name'}

                                        />
                                        {errors.guardianName1 && <span className="error-message">{errors.guardianName1}</span>}
                                    </div>
                                    <div className="form-group">
                                        <label htmlFor="guardianContact">{formData.parentalStatus === "has one" ? 'Parent Contact' : 'Guardian Contact'} *</label>
                                        <input
                                            type="tel"
                                            id="guardianContact1"
                                            ...
                                            pattern="[0-9]{10}"
                                        />
                                        {errors.guardianContact1 && <span className="error-message">{errors.guardianContact1}</span>}
                                    </div>
                                </>
                            )
                        }
                        ...
                        {
                            (formData.parentalStatus && formData.parentalStatus) === "has both" && (
                                <>
                                    <div className="form-group">
                                    ...
                                    <div className="form-group">
                                        ...
                                        />
                                        {errors.guardianName2 && <span className="error-message">{errors.guardianName2}</span>}
                                    </div>
                                    <div className="form-group">
                                        ...
                                        <input
                                            ...
                                            pattern="[0-9]{10}"
                                        />
                                        {errors.guardianContact2 && <span className="error-message">{errors.guardianContact2}</span>}
                                    </div>
                                </>
                            )
                        }
                        ...
                        <div className="form-group">
                            ...
                            {errors.facialData && <span className="error-message">{errors.facialData}</span>}
                        </div>
                    </div>
                )}
                                ...
                                <div className="form-section">
                                    ...
                                    <div className="form-row">
                                        <div className="form-group">
                                            ...
                                            <select
                                                ...
                                                onChange={(e) => handleMedicalHistoryNestedChange(index, 'currentStatus', 'status', e.target.value)}
                                        ...
                                        <div className="form-group">
                                            ...
                                            <input
                                                ...
                                                onChange={(e) => handleMedicalHistoryNestedChange(index, 'currentStatus', 'date', e.target.value)}
                                    ...
                                    <div className="form-group">
                                        ...
                                        <textarea
                                            ...
                                            onChange={(e) => handleMedicalHistoryNestedChange(index, 'currentStatus', 'notes', e.target.value)}
                                ...
                                <div className="form-row">
                                    <div className="form-group">
                                        ...
                                        <input
                                            type="file"
                                            multiple
                                            onChange={(e) => handleMedicalHistoryFileChange(index, 'prescriptions', e.target.files)}
                                        ...
                                        <div className="file-list">
                                            ...
                                            {files.medicalHistoryFiles[index]?.prescriptions?.map((file, fileIndex) => (
                                                <div key={`new-${fileIndex}`} className="file-item">
                                                    <span>{file.name}</span>
                                                    <button
                                                        type="button"
                                                        onClick={() => handleRemoveMedicalHistoryFile(index, 'prescriptions', fileIndex)}
                                                    >
                                                        ✕
                                                    </button>
                                                </div>
                                            ...
                                            {mode === 'edit' && history.prescriptions?.map((file, fileIndex) => (
                                                <div key={`existing-${fileIndex}`} className="file-item existing">
                                                    <span>{file.name}</span>
                                    ...
                                    <div className="form-group">
                                        ...
                                        <input
                                            type="file"
                                            multiple
                                            onChange={(e) => handleMedicalHistoryFileChange(index, 'otherAttachments', e.target.files)}
                                        ...
                                        <div className="file-list">
                                            {/* Show new files selected */}
                                            {files.medicalHistoryFiles[index]?.otherAttachments?.map((file, fileIndex) => (
                                                <div key={`new-${fileIndex}`} className="file-item">
                                                    <span>{file.name}</span>
                                                    <button
                                                        type="button"
                                                        onClick={() => {
                                                            const newFiles = [...files.medicalHistoryFiles[index].otherAttachments];
                                                            newFiles.splice(fileIndex, 1);
                                                            setFiles(prev => ({
                                                                ...prev,
                                                                medicalHistoryFiles: {
                                            ...
                                            {/* Show existing files from edit mode */}
                                            {mode === 'edit' && history.otherAttachments?.map((file, fileIndex) => (
                                                <div key={`existing-${fileIndex}`} className="file-item existing">
                                                    <span>{file.name}</span>
                                                    <a href={file.url} target="_blank" rel="noopener noreferrer" className="view-btn">
                                                        View
