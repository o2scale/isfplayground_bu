...
export const RBACProvider = ({ children }) => {
    ...
    useEffect(() => {
        const fetchPermissions = async () => {
            ...
            try {
                ...
                const response = await axios.get('http://localhost:5001/api/roles/getAllRolePermissions');
                ...
                if (response.data) {
                    ...
                    // If the response has a success property and roles array
                        ...
                        console.log('Available roles:', response.data.roles.map(r => r.roleName.toLowerCase()));
    ...
    const hasPermission = (module, action) => {
        ...
        if (module === "Role Management" && action == "Read") {
            console.log('black baccc', module, '-', action, ':', permissions[module].includes(action))
        }
    ...
    const getRolePermissions = (roleName) => {
        const role = allRoles.find(r => r.roleName.toLowerCase() === roleName.toLowerCase());
