...
const { getUserIdsByRoles, getUserSportsTasksByFilters, getTasksByBalagruhaIdAndFilter, getTotalStudentsCountByBalagruhaId, getSportsTasksCountByBalagruhaId, getStudentsWithSportsTaskByBalagruhaId, getMusicTasksCountByBalagruhaId } = require("../data-access/User");
const { getTrainingSessionsCount, getTrainingSessionsByBalagruhaAndDate } = require("../data-access/trainingSession");
const Task = require("./task");
const { updateTaskItemById, updateTaskById, findTaskById } = require("../data-access/task");
...
class MusicTask {
    ...
    static async getMusicInsights({ balagruhaId, date = new Date() }) {
        try {
            ...
            let studentCount = await getTotalStudentsCountByBalagruhaId({ balagruhaIds: balagruhaId })
            let trainingSessionCount = await getTrainingSessionsCount({ balagruhaIds: balagruhaId, type })
            let activeTaskCount = await getMusicTasksCountByBalagruhaId({ balagruhaIds: balagruhaId })
            let todaysTrainingSessions = await getTrainingSessionsByBalagruhaAndDate({ balagruhaIds: balagruhaId, date, type: ["music"] })
            let todaysTasks = await getStudentsWithSportsTaskByBalagruhaId({ balagruhaIds: balagruhaId, date })
    ...
    static async addOrUpdateMusicTaskAttachment({ taskId, attachments, createdById, isOfflineReq = false }) {
        try {
    ...
    static async addCommentsToMusicTask({ taskId, comments, createdById, attachments, isOfflineReq = false }) {
        try {
