...
router.get('/user/:userId', authenticate, authorize('Task Management', 'Read'), taskController.getAllTasksForUser);
// API for task management page 
router.get("/overview", authenticate, authorize('Task Management', 'Read'), taskController.getTaskOverview)
// API for fetch the task list by balagruha id and filter 
router.post("/all/list", authenticate, authorize('Task Management', 'Read'), taskController.getTaskListByBalagruhaIdAndFilter)
// API for fetch the task overview details by balagruha id
router.get("/overview/details/:balagruhaId", authenticate, authorize('Task Management', 'Read'), taskController.getTaskOverviewDetailsByBalagruhaId)
// API for update task status 
router.put("/status/:id", authenticate, authorize('Task Management', 'Update'), taskController.updateTaskStatus)
// API for add comments on task 
router.post("/comment/:taskId", authenticate, authorize('Task Management', 'Update'), upload.fields([
...
router.put("/attachments/:taskId", authenticate, authorize('Task Management', 'Update'), upload.fields([
	{ name: 'attachments', maxCount: 5 }, // Up to 5 files for facialData
]), taskController.addOrUpdateTaskAttachment)
// API for delete the task attachment
router.delete("/attachments/:taskId/:attachmentId", authenticate, authorize('Task Management', 'Update'), taskController.deleteTaskAttachment)
