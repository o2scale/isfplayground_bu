...
const { createUser, findUsersByRoleAndBalagruhaId, getUserInfo, updateUserPasswordByAdmin, assignBalagruhaToUser, updateUserDetailsById, deleteUserById, getUserListByAssignedBalagruhaByRole } = require('../services/user');
...
exports.createUserV1 = async (req, res) => {
    try {
        ...
        // return
        logger.info({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: logData }, `Request received for user registration`);
        ...
        if (result.success) {
            ...
            res.status(400).json({ success: false, data: {}, message: result.message });
...
exports.createStudentMedicalRecords = async (req, res) => {
    try {
        logger.info({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `Request received for user registration`);
        let result = await Student.createStudentMedicalRecords(req.body)
        if (result.success) {
            logger.info({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `User registered successfully`);
            res.status(201).json(result);
        } else {
            errorLogger.error({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `Error occurred while user registration: ${error.message}`);
        }
    } catch (error) {
        res.status(400).json({ message: error.message });
    }
...
exports.getUserManagementOverviewDetails = async (req, res) => {
    try {
        ...
        logger.info({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `Request received for fetching the user overview for user management`);
        let result = await Student.getStudentListByBalagruhaId({ balagruhaId })
        if (result.success) {
            logger.info({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `Successfully processed the request for fetching the user overview for user management`);
            res.status(201).json(result);
        } else {
            errorLogger.error({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `Error occurred while fetching the user overview for user management: ${error.message}`);
            res.status(400).json(result);
        }
    } catch (error) {
        errorLogger.error({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `Error occurred while processing the request for fetching the user overview details: ${error.message}`);
        res.status(400).json({ message: error.message });
    }
}
...
exports.getStudentListByBalagruhaId = async (req, res) => {
    try {
        logger.info({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `Request received for fetching the student list by balagruha id`);
        let result = await Student.getStudentListByBalagruhaId(req.body)
        if (result.success) {
            logger.info({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `Successfully processed the request for fetching the student list by balagruha id`);
            res.status(201).json(result);
        } else {
            errorLogger.error({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `Error occurred while fetching the student list by balagruha id: ${error.message}`);
            res.status(400).json(result);
        }
    } catch (error) {
        errorLogger.error({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `Error occurred while processing the request for fetching the student list by balagruha id: ${error.message}`);
        res.status(400).json({ message: error.message });
    }
}
...
exports.createStudentAttendance = async (req, res) => {
    try {
        logger.info({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `Request received for creating attendance for the student`);
        let result = await Attendance.saveAttendance(req.body)
        if (result.success) {
            logger.info({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `Successfully processed the request for creating attendance for the student`);
            res.status(201).json(result);
        } else {
            errorLogger.error({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `Error occurred while creating attendance for the student: ${error.message}`);
            res.status(400).json(result);
        }
    } catch (error) {
        errorLogger.error({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `Error occurred while processing the request for creating attendance for the student: ${error.message}`);
        res.status(400).json({ message: error.message });
    }
}
...
exports.getStudentListByBalagruhaIdWithAttendance = async (req, res) => {
    try {
        ...
        let balagruhaId = req.params.balagruhaId;
        logger.info({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `Request received for fetching the student list in balagruha with the attendance by given date`);
        let result = await Student.getStudentListByBalagruhaIdWithAttendance({ balagruhaId, date })
        if (result.success) {
            logger.info({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `Successfully processed the request for fetching the student list in balagruha with the attendance by given date`);
            res.status(201).json(result);
        } else {
            errorLogger.error({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `Error occurred while fetching the student list in balagruha with the attendance by given date: ${error.message}`);
            res.status(400).json(result);
        }
    } catch (error) {
        errorLogger.error({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `Error occurred while processing the request for fetching the student list in balagruha with the attendance by given date: ${error.message}`);
        res.status(400).json({ message: error.message });
    }
}
...
exports.facialLogin = async (req, res) => {
    try {
        ...
        logger.info({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `Request received for facial login`);
        let result = await Student.faceLogin(req.body)
        if (result.success) {
            logger.info({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `Successfully processed the request for facial login`);
            res.status(201).json(result);
        } else {
            errorLogger.error({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `Error occurred while facial login`);
            res.status(400).json(result);
        }
    } catch (error) {
        console.log('error', error)
        errorLogger.error({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `Error occurred while processing the request for facial login: ${error.message}`);
        res.status(400).json({ message: error.message });
    }
}
...
exports.getUsersByRoleAndBalagruhaId = async (req, res) => {
    try {
        ...
        logger.info({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: { role, balagruhaId } },
...
const extractMedicalHistory = (req) => {
    ...
    Object.keys(req.body).forEach((key) => {
        ...
        if (match) {
            ...
            if (field.startsWith("currentStatus.")) {
                medicalHistory[index].currentStatus[field.replace("currentStatus.", "")] = req.body[key];
            } else {
    ...
    if (req.files) {
        ...
        for (let i = 0; i < req.files.length; i++) {
            ...
            const match = file.fieldname.match(/^medicalHistory\[(\d+)\]\.(prescriptions|otherAttachments)$/);
...
exports.updateUserPassword = async (req, res) => {
    try {
        ...
        if (role !== UserTypes.ADMIN) {
            return res.status(403).json({ success: false, message: "You don't have permission to perform this action. Only admin users can update passwords." });
        }
        logger.info({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `Request received for updating user password`);
        let result = await updateUserPasswordByAdmin(req.body)
        if (result.success) {
            logger.info({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `Successfully processed the request for updating user password`);
            res.status(201).json(result);
        } else {
            errorLogger.error({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `Error occurred while updating user password: ${error.message}`);
            res.status(400).json(result);
        }
    } catch (error) {
        errorLogger.error({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `Error occurred while processing the request for updating user password: ${error.message}`);
        res.status(400).json({ message: error.message });
    }
}
...
exports.assignBalagruhaToUser = async (req, res) => {
    try {
        logger.info({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `Request received for assigning balagruha to the user`);
        let result = await assignBalagruhaToUser(req.body)
        if (result.success) {
            logger.info({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `Successfully processed the request for assigning balagruha to the user`);
            res.status(201).json(result);
        } else {
            errorLogger.error({ clientIP: req.socket.remoteAddress, method: req.method, api: req.originalUrl, data: req.body }, `Error occurred while assigning balagruha to the user: ${error.message}`);
            res.status(400).json(result);
        }
    } catch (error) {
    ...
    try {
        ...
        if (role !== UserTypes.ADMIN) {
            return res.status(403).json({
                success: false,
                message: "You don't have permission to perform this action. Only admin users can delete users."
            });
        }
