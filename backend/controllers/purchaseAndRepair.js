...
const repairRequestController = {
    ...
    createRepairRequest: async (req, res) => {
        try {
            ...
            const newRepairRequest = await RepairRequest.createRepairRequest(repairData);
    ...
    getAllRepairRequests: async (req, res) => {
        try {
            const { page = 1, limit = 10, sort = 'createdAt', order = 'desc', urgency } = req.query;
    ...
    updateRepairRequest: async (req, res) => {
        try {
            ...
            const updatedRequest = await RepairRequest.updateRepairRequest(id, updateData);
    ...
    createPurchaseOrder: async (req, res) => {
        try {
            ...
            const newPurchaseOrder = await PurchaseOrder.createPurchaseOrder(purchaseOrderData);
    ...
    updatePurchaseOrder: async (req, res) => {
        try {
            ...
            // Handle file attachments if any
            if (req.files && req.files.length > 0) {
                const result = await PurchaseOrder.processAttachments(req.files, req.user._id, isOfflineReq);
            ...
            const updatedOrder = await PurchaseOrder.updatePurchaseOrder(id, updateData);
    ...
    updatePurchaseOrderStatus: async (req, res) => {
        try {
            ...
            const updatedOrder = await PurchaseOrder.updatePurchaseOrder(id, { status });
    ...
    getPurchaseManagerOverview: async (req, res) => {
        try {
            const overviewData = await PurchaseOrder.getPurchaseManagerOverview(req.user._id);
